// MemoryHolderFactory_Partial.h
// Copyright (c) 2013 The Foundry Visionmongers Ltd.  All Rights Reserved.
 
#if !(defined DDImage_MemoryHolderFactory_H) || (defined DDImage_MemoryHolderFactory_Partial_H)
  #error Only to be included by MemoryHolderFactory.h
#endif

#ifndef DDImage_MemoryHolderFactory_Partial_H
#define DDImage_MemoryHolderFactory_Partial_H
...
#define FN_MEMORYHOLDERFACTORY_SPECIALIZATION                                 \
  template<FN_TEMPLATE_PARAMETER_DECLARATIONS>                                \
  class MemoryHolderFactory<FN_TEMPLATE_PARAMETERS>                           \
  {                                                                           \
  public:                                                                     \
    MemoryHolderFactory() {}                                                  \
    ~MemoryHolderFactory() {}                                                 \
                                                                              \
    mFnDeprecatedInNuke12Func(                                                \
    "MemoryHolderFactory<Type, Args...> has been replaced with MemHolderFactory<Type>. Use as a drop-in replacement", \
    static MemoryHolderT* create(FN_FUNCTION_ARGUMENT_DECLARATIONS);          \
    )                                                                         \
                                                                              \
  protected:                                                                  \
    /* Not implemented */                                                     \
    ...
    MemoryHolderFactory& operator=(const MemoryHolderFactory& rhs);           \
                                                                              ...
                                                                              \
  template<FN_TEMPLATE_PARAMETER_DECLARATIONS>                                \
  MemoryHolderT* MemoryHolderFactory<FN_TEMPLATE_PARAMETERS>::create(         \
    FN_FUNCTION_ARGUMENT_DECLARATIONS                                         \
  )                                                                           \
  {                                                                           \
    return new MemoryRegistrant<FN_TEMPLATE_PARAMETERS>(FN_FUNCTION_ARGUMENTS);\
  }
...
#endif // DDImage_MemoryHolderFactory_Partial_H
